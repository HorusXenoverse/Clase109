...
#Para obtener el tamaño de la pantalla de la grabación
width  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) 
height  = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

#Para obtener el tamaño de la pantalla del escritorio
...
#Es el equivalente a un gameState
pinch=False

# Definir una función para contar dedos
def countFingers(image, hand_landmarks, handNo=0):

    #En python debes especificar si es una variable global y la metes en la función donde la vas a ocupar
	global pinch
	...
	if hand_landmarks:
		...
		dedoIndiceX = int((landmarks[8].x)*width )
		dedoIndiceY = int((landmarks[8].y)*height)

		dedoPulgarX = int((landmarks[4].x)*width )
		dedoPulgarY = int((landmarks[4].y)*height)

		cv2.line(image, (dedoIndiceX,dedoIndiceY), (dedoPulgarX, dedoPulgarY), (255,0,0), 2 )
		

		# Dibujar un círculo en el centro de la línea entre la punta del dedo y la punta del pulgar
		circuloX = int((dedoIndiceX + dedoPulgarX)/2)
		circuloY = int((dedoIndiceY + dedoPulgarY)/2)

		cv2.circle(image, (circuloX, circuloY), 2, (0,0,255), 2)
		

		# Calcular la distancia entre la punta del dedo y la punta del pulgar
		distance = math.sqrt( ((dedoIndiceX - dedoPulgarX)**2) + ((dedoIndiceY - dedoPulgarY)**2)  )
		

		# Establecer la posición del mouse en la pantalla relativa al tamaño de la ventana del output
		mouseVirtualX = (circuloX / width)*screen_width
		mouseVirtualY = (circuloY / height)*screen_height

		mouse.position = (mouseVirtualX, mouseVirtualY)
